...
def acc_test(loader, model, device):
    ...
    with torch.no_grad():  # 不计算梯度
        for i, (x, y) in enumerate(loader):  # 获取输入与输出
            x = x.reshape(-1, 28 * 28).to(device)  # 将图片转换为一维张量
            y = y.to(device)
            pre = model(x)  # 预测
...
def train(path, input_, hidden=256, output_=10, num=1, lr=0.01, device='cpu', epochs=30):
    """
    训练模型
    :param path: 数据存放路径
    ...
    # 创建一个全连接网络的对象
    model = classmodel.Model(input_, hidden, output_, num)

    # 创建优化器
    optimizer = torch.optim.Adam(model.parameters(), lr=lr)  # 使用Adam优化器

    # 获取数据
    train_loader, test_loader = get_dataset(path)
    ...
    for epoch in range(epochs):
        for i, (x, y) in enumerate(train_loader):  # 获取输入与输出
            x = x.reshape(-1, 28 * 28).to(device)  # 将图片转换为一维张量并移动到设备上
