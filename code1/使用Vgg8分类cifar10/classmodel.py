...
class Vgg(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        self.vgg1 = VggBlock(in_channels, out_channels, 2)  # VGG块
        self.vgg2 = VggBlock(out_channels, out_channels, 3)
        self.vgg3 = VggBlock(out_channels, out_channels, 3)

        # 输入：将经历VGG块的输出展平乘以输出通道数
        self.linear1 = nn.Linear(32 * 32 * 48, 10)  # 线性层

    def forward(self, x):  # 前向传播
        x = self.vgg1(x)
        x = self.vgg2(x)
        x = self.vgg3(x)
        x = self.linear1(x.view(-1, 32 * 32 * 48))
        return x
